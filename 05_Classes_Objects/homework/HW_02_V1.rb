...
class Game
	...
	def initialize(name)
		...
		#Initialize the Secret Number class 
		@game = Secret_Number.new

		#welcome the player
		#puts `clear`
		puts "Welcome #{@player.name}! Guess the Secret Number, it is between 1 and 10."
	...
	def play_round
		while @guess_remain > 0
			...
			abort("You are out of guesses. Game over!")
	end
	...
	def check_ans(user_guess)
		...
		#see if the answer is correct, high or low
			...
			abort("You win!! Well played #{@player.name}")
			end
		end
	end	
end
